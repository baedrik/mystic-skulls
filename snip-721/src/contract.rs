...
pub fn init<S: Storage, A: Api, Q: Querier>(
    ...
    let admins = vec![admin_raw];
    save(&mut deps.storage, ADMINS_KEY, &admins)?;
    save(&mut deps.storage, MINTERS_KEY, &admins)?;
    ...
    let mut messages: Vec<CosmosMsg> = if let Some(callback) = msg.post_init_callback {
    ...
    let server_inf = ServerInfo {
        default: 0u16,
        viewing_key: vk.0,
    };
    save(&mut deps.storage, SVG_INFO_KEY, &server_inf)?;

    // store the svg servers if given
    if let Some(svrs) = msg.svg_servers {
        if !svrs.is_empty() {
            let mut reg = Registry::new(&deps.storage, PREFIX_SERVER_REGISTRY)?;
            for svr in svrs.into_iter() {
                add_svr(
                    deps,
                    &mut reg,
                    svr,
                    &mut messages,
                    false,
                    server_inf.viewing_key.clone(),
                )?;
            }
            // save the registry count
            reg.save(&mut deps.storage)?;
        }
    }
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    let response = match msg {
        ...
        HandleMsg::AddAdmins { admins, .. } => try_process_auth_list(
            deps,
            &env.message.sender,
            &config,
            ContractStatus::StopTransactions.to_u8(),
        ...
        HandleMsg::RemoveAdmins { admins, .. } => try_process_auth_list(
            deps,
            &env.message.sender,
            &config,
            ContractStatus::StopTransactions.to_u8(),
        ...
        HandleMsg::AddSvgServer { svg_server, .. } => try_add_server(
            deps,
            &env.message.sender,
            &config,
            ContractStatus::StopTransactions.to_u8(),
        ...
        HandleMsg::SetDefaultSvgServer { svg_server, .. } => try_set_default_svr(
            deps,
            &env.message.sender,
            &config,
            ContractStatus::StopTransactions.to_u8(),
...
pub fn try_set_image_info<S: Storage, A: Api, Q: Querier>(
    ...
    if !minters.contains(&sender_raw) {
        return Err(StdError::generic_err(
            "Only designated minters are allowed to set image info",
        ));
    }
    ...
    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
        data: Some(to_binary(&HandleAnswer::SetImageInfo { status: Success })?),
    })
}
...
/// * `config` - a reference to the Config
/// * `priority` - u8 representation of highest ContractStatus level this action is permitted
/// * `server` - new svg server contract info
pub fn try_add_server<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    sender: &HumanAddr,
    config: &Config,
    priority: u8,
    ...
    Ok(HandleResponse {
        messages,
        log: vec![],
        data: Some(to_binary(&HandleAnswer::AddSvgServer { status: Success })?),
    })
}

/// Returns HandleResult
///
/// sets a new default svg server
///
/// # Arguments
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
...
/// * `config` - a reference to the Config
/// * `priority` - u8 representation of highest ContractStatus level this action is permitted
/// * `server` - new default svg server contract address
pub fn try_set_default_svr<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    sender: &HumanAddr,
    config: &Config,
    priority: u8,
    ...
    let mut svr_inf: ServerInfo = load(&deps.storage, SVG_INFO_KEY)?;
    let raw = deps.api.canonical_address(server)?;
    let idx = Registry::<StoreContractInfo>::get_idx(
        &deps.storage,
        raw.as_slice(),
        PREFIX_SERVER_REGISTRY,
    )?;
    if idx != svr_inf.default {
        svr_inf.default = idx;
        save(&mut deps.storage, SVG_INFO_KEY, &svr_inf)?;
    }

    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
        data: Some(to_binary(&HandleAnswer::SetDefaultSvgServer {
            status: Success,
        })?),
    })
}
...
pub fn query_debug<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    token_id: &str,
) -> QueryResult {
    let map2idx = ReadonlyPrefixedStorage::new(PREFIX_MAP_TO_INDEX, &deps.storage);
    ...
    if let Some(idx) = may_idx {
        ...
        let svr_meta = get_debug(deps, &token_key)?;
...
fn get_debug<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    token_key: &[u8],
...
/// Returns QueryResult displaying a token's ImageInfo
///
/// # Arguments
///
/// * `deps` - reference to Extern containing all the contract's external dependencies
...
/// * `token_id` - ID of token whose ImageInfo to display
fn query_image_info<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    ...
    if !minters.contains(&viewer_raw) {
        return Err(StdError::generic_err(
            "This is a minter command and can only be run from an authorized minter address",
        ));
    }
    let map2idx = ReadonlyPrefixedStorage::new(PREFIX_MAP_TO_INDEX, &deps.storage);
    let idx: u32 = may_load(&map2idx, token_id.as_bytes())?
...
/// * `viewer` - optional address and key making an authenticated query request
/// * `from_permit` - address derived from an Owner permit, if applicable
fn query_default_server<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
) -> QueryResult {
    ...
    to_binary(&QueryAnswer::DefaultSvgServer {
        default_svg_server: raw.into_humanized(&deps.api)?,
    })
}

/// Returns QueryResult listing all svg server contracts
///
/// # Arguments
///
/// * `deps` - reference to Extern containing all the contract's external dependencies
...
/// * `page_size` - optional maximum number of contracts to display
fn query_svg_servers<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    viewer: Option<ViewerInfo>,
    from_permit: Option<CanonicalAddr>,
    page: Option<u16>,
    ...
    let reg = Registry::<StoreContractInfo>::new(&deps.storage, PREFIX_SERVER_REGISTRY)?;
    let (count, raws) = reg.display(&deps.storage, page, page_size)?;
    let svg_servers = raws
        .into_iter()
...
/// Returns HandleResult
///
/// updates the admin or minter authorization list
///
/// # Arguments
///
/// * `deps` - a mutable reference to Extern containing all the contract's external dependencies
...
/// * `is_admin` - true if this update is to the admin list
fn try_process_auth_list<S: Storage, A: Api, Q: Querier>(
    ...
    let (mut current_list, key) = if is_admin {
        (admins, ADMINS_KEY)
    } else {
        (
    ...
    let resp = if is_add {
        if is_admin {
            HandleAnswer::AddAdmins { status: Success }
        } else {
            HandleAnswer::AddMinters { status: Success }
        }
    } else if is_admin {
        HandleAnswer::RemoveAdmins { status: Success }
    } else {
        HandleAnswer::RemoveMinters { status: Success }
    };
    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
...
///
/// * `deps` - mutable reference to Extern containing all the contract's external dependencies
/// * `reg` - a mutable reference to the svg server registry
/// * `server` - new svg server contract info
/// * `messages` - a mutable reference to the list of callback msgs
/// * `save_cnt` - true if the registry count should be saved
/// * `viewing_key` - viewing key to set with the new server contract
fn add_svr<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    reg: &mut Registry<StoreContractInfo>,
    server: ContractInfo,
    ...
    let raw = StoreContractInfo {
        address: deps.api.canonical_address(&server.address)?,
        code_hash: server.code_hash,
    };
    // if the server was not already in the registry
    if reg.add(&mut deps.storage, &raw, save_cnt)? {
        // set the viewing key with the svg server
