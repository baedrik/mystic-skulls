...
mod tests {
    ...
    use crate::state::{
        ...
        PermissionType, ServerInfo, ADMINS_KEY, CONFIG_KEY, MINTERS_KEY, PREFIX_ALL_PERMISSIONS,
        PREFIX_AUTHLIST, PREFIX_INFOS, PREFIX_MAP_TO_ID, PREFIX_MAP_TO_INDEX, PREFIX_OWNER_PRIV,
        PREFIX_PRIV_META, PREFIX_PUB_META, PREFIX_RECEIVERS, PREFIX_SERVER_REGISTRY,
        PREFIX_VIEW_KEY, SVG_INFO_KEY,
    };
    ...
    fn init_helper_default() -> (
        ...
        let server = ContractInfo {
            address: HumanAddr("server".to_string()),
            code_hash: "server hash".to_string(),
        };
    ...
    fn test_init_sanity() {
        ...
        let admins: Vec<CanonicalAddr> = load(&deps.storage, ADMINS_KEY).unwrap();
        let expect_admins = vec![deps
            .api
        ...
        assert_eq!(admins, expect_admins);
        ...
        let expected = server.into_store(&deps.api).unwrap();
        assert_eq!(
            Registry::<StoreContractInfo>::get_at(&deps.storage, 0u16, PREFIX_SERVER_REGISTRY)
                .unwrap(),
            expected
        );
        let server_inf: ServerInfo = load(&deps.storage, SVG_INFO_KEY).unwrap();
        assert_eq!(server_inf.default, 0u16);
        ...
        let mut init_response = init(&mut deps, env, init_msg).unwrap();
        let hook = init_response.messages.swap_remove(0);
        assert_eq!(
            hook,
    ...
    fn test_add_svg_server() {
        ...
        let svg_server = ContractInfo {
            address: HumanAddr("server2".to_string()),
            code_hash: "hash2".to_string(),
        };

        // test adding a server when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopAll,
            padding: None,
        };
        ...
        // you can still add a server when transactions are stopped
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopTransactions,
            padding: None,
        };
        ...
        // test non admin trying to add a server
        let handle_msg = HandleMsg::AddSvgServer {
            svg_server: svg_server.clone(),
            padding: None,
        };
        ...
        let expected = svg_server.get_store(&deps.api).unwrap();
        // sanity check
        // verify the server we will add is not already in the list
        ...
        let registry =
            Registry::<StoreContractInfo>::new(&deps.storage, PREFIX_SERVER_REGISTRY).unwrap();
        assert_eq!(
            registry
                .self_get_idx(&deps.storage, expected.address.as_slice())
                .unwrap(),
            1u16
        );
        assert_eq!(registry.self_get_at(&deps.storage, 1u16).unwrap(), expected);
    }

    // test set default svg server
    #[test]
    fn test_seet_default_svg_server() {
        let (init_result, mut deps) = init_helper_default();
        assert!(
            init_result.is_ok(),
            "Init failed: {}",
        ...
        // test setting a default when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopAll,
            padding: None,
        };
        ...
        let handle_msg = HandleMsg::SetDefaultSvgServer {
            svg_server: svg_server.address.clone(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        ...
        // you can still set a default when transactions are stopped
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopTransactions,
            padding: None,
        };
        ...
        // test non admin trying to set a default
        let handle_msg = HandleMsg::SetDefaultSvgServer {
            svg_server: svg_server.address.clone(),
            padding: None,
        };
        ...
        // test when the server is not in the registry
        let handle_msg = HandleMsg::SetDefaultSvgServer {
            svg_server: HumanAddr("not a server".to_string()),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        assert!(error.contains("Attempting to get_idx of item not in the registry"));
        ...
        let server_inf: ServerInfo = load(&deps.storage, SVG_INFO_KEY).unwrap();
        assert_eq!(server_inf.default, 1u16);
    }
    ...
    fn test_add_admins() {
        ...
        // test adding admins when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopAll,
            padding: None,
        };
        ...
        let handle_msg = HandleMsg::AddAdmins {
            admins: admins.clone(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        ...
        // you can still add admins when transactions are stopped
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopTransactions,
            padding: None,
        };
        ...
        // test non admin trying to add admins
        let handle_msg = HandleMsg::AddAdmins {
            admins: admins.clone(),
            padding: None,
        };
        ...
        // sanity check
        let cur_admins: Vec<CanonicalAddr> = load(&deps.storage, ADMINS_KEY).unwrap();
        let alice_raw = deps
            .api
            .canonical_address(&HumanAddr("alice".to_string()))
            .unwrap();
        ...
        let admin_raw = deps
            ...
            .canonical_address(&HumanAddr("admin".to_string()))
            .unwrap();
        // verify the admins we will add are not already in the list
        assert!(!cur_admins.contains(&alice_raw));
        assert!(!cur_admins.contains(&bob_raw));
        let handle_msg = HandleMsg::AddAdmins {
            admins,
            padding: None,
        };
        let _handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        // verify the new admins were added
        let cur_admins: Vec<CanonicalAddr> = load(&deps.storage, ADMINS_KEY).unwrap();
        assert_eq!(cur_admins.len(), 3);
        assert!(cur_admins.contains(&alice_raw));
        assert!(cur_admins.contains(&bob_raw));
        assert!(cur_admins.contains(&admin_raw));
    ...
    fn test_remove_admins() {
        ...
        // test removing admins when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopAll,
            padding: None,
        };
        ...
        let handle_msg = HandleMsg::RemoveAdmins {
            admins: admins.clone(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        ...
        // you can still remove admins when transactions are stopped
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopTransactions,
            padding: None,
        };
        ...
        // test non admin trying to remove admins
        let handle_msg = HandleMsg::RemoveAdmins {
            admins: admins.clone(),
            padding: None,
        };
        ...
        // verify the new admins were added
        let cur_admins: Vec<CanonicalAddr> = load(&deps.storage, ADMINS_KEY).unwrap();
        assert_eq!(cur_admins.len(), 4);
        assert!(cur_admins.contains(&alice_raw));
        assert!(cur_admins.contains(&bob_raw));
        assert!(cur_admins.contains(&charlie_raw));
        assert!(cur_admins.contains(&admin_raw));
        ...
        // let's remove the last one
        let handle_msg = HandleMsg::RemoveAdmins {
            admins: vec![HumanAddr("admin".to_string())],
            padding: None,
        };
        let _handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        // verify the admins were removed
        let cur_admins: Vec<CanonicalAddr> = load(&deps.storage, ADMINS_KEY).unwrap();
        assert!(cur_admins.is_empty());
    }
